...
const resolveShareCode = () => {
  const pathnameSegments = globalThis.location.pathname
    .split("/")
    .filter(Boolean);
  ...
  const redirectParam = new URLSearchParams(globalThis.location.search).get(
    "redirect_url",
  );
...
async function base<T>(
  ...
  if (isMarketplaceAPI)
    (headers as any).set(
      "X-Dify-Version",
      !IS_MARKETPLACE ? APP_VERSION : "999.0.0",
    );
